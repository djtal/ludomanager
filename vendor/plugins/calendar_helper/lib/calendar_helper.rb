...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    day_names = options[:day_name].dup
    ...
    cal << %(<th colspan="#{colspan}" class="#{options[:month_name_class]}">#{options[:month_name][options[:month]]}</th>)
    ...
    beginning_of_week(first, first_weekday).upto(first - 1) do |d|
      ...
      if options[:accessible]
        cal << %(">#{d.day}<span class="hidden"> #{options[:month_name][d.month]}</span></td>)
      else
        cal << %(">#{d.day}</td>)
      end
    ...
    (last + 1).upto(beginning_of_week(last + 7, first_weekday) - 1)  do |d|
      ...
      cal << " weekendDay" if weekend?(d)
      if options[:accessible]
        cal << %(">#{d.day}<span class='hidden'> #{options[:month_name][d.mon]}</span></td>)
      else
        cal << %(">#{d.day}</td>)        
      end
